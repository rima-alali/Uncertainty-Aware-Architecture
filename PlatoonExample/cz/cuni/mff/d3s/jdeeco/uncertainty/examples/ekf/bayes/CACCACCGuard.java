...
//		System.out.println(insDist.keySet());
		...
		Boolean cond1 = distance.getMinBound() < vehDiseredDistance || diffbrakingDistance.getMinBound() <= 0;
		Boolean cond2 = StatisticalOperators.below(distree, vehDiseredDistance, delayDistanceMean.intValue()) || StatisticalOperators.below(bdistree, 0.0, delayDistanceMean.intValue());
		Boolean cond3 = StatisticalOperators.fbelow(distree, vehDiseredDistance, delayDistanceMean.intValue(),delayDistanceMean.intValue()) || StatisticalOperators.fbelow(bdistree, 0.0, delayDistanceMean.intValue(),delayDistanceMean.intValue());
...
//		TreeMap<Double, Double> corrACC = BayesianOperators.fcorr(distreeACC, outsACC, 7.0, 1000, 0);
		...
		Boolean cond4 = false;//BayesianOperators.fbelowcorr(distreeCACC, outsCACC, 5.0, distreeACC, outsACC, 7.0, 10, 0);
...
//		System.err.println("cond ... "+brakingDistance+" > "+distance.getMinBound()+"  "+(brakingDistance > distance.getMinBound()));
	...
	private void setLogger() {
		boolean append = true;
		try {
			FileHandler handler = new FileHandler("./logs/bayes.csv", append);
