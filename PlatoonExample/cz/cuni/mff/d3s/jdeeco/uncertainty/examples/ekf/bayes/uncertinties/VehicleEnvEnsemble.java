...
public class VehicleEnvEnsemble {
	...
	public static void map(
			...
			@In("coord.vehRouteSlopes") LookupTable vehRouteSlopes,
			@In("coord.vehMass") Double vehMass,
			@In("coord.vehPosACC") State vehPosACC,
			@In("coord.vehSpeedACC") State vehSpeedACC, 
			@InOut("coord.vehAccelACC") ParamHolder<State> vehAccelACC,
			@In("coord.vehGasACC") State vehGasACC,
			@In("coord.vehBrakeACC") State vehBrakeACC,
			...
			@InOut("member.ePosACC") ParamHolder<State> ePosACC, 
			@InOut("member.eSpeedACC") ParamHolder<State> eSpeedACC,
			@In("member.eAccelACC") State eAccelACC,
			@InOut("member.eGasACC") ParamHolder<State> eGasACC, 
			@InOut("member.eBrakeACC") ParamHolder<State> eBrakeACC,
...
//		System.err.println(" ens ..."+vehAccel.value.getData());
		...
		ePosACC.value.setData(vehPosACC.getDataRange());
		eSpeedACC.value.setData(vehSpeedACC.getDataRange());
		eGasACC.value.setData(vehGasACC.getDataRange());
		eBrakeACC.value.setData(vehBrakeACC.getDataRange());
		vehAccelACC.value.setData(eAccelACC.getDataRange());
