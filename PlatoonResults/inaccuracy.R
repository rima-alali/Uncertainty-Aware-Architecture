...
#Read the data
guard <- read.csv(file="./inaccuracy.csv",sep=",",head=TRUE)   
...
normalCondition <- ggplot(v3, aes(x = as.numeric(as.character(v3$vehPosValue))))+
                                      ylim("","false","true") +
                                      theme(text = element_text(size=25))+
...
minCondition <- ggplot(v3, aes(x = as.numeric(as.character(v3$vehPosValue))))+
                                   ylim("","false","true") +
                                   theme(text = element_text(size=25))+
...
grid.arrange(normalCondition, minCondition, ncol = 1, nrow = 2)
